...
void LineTracer::run() {
	...
	if( mSection == RunManager::STRAIGHT_ZONE){
		mPidController->setPID(1.0, 0, 10.0);
		mBalancingWalker->setCommand(80, direction);	//■■速度は暫定
		mBalancingWalker->run();
  ...
  #endif
  #if 0
		int16_t gyro_angler_velocity = mGyroSensor->getAnglerVelocity();

		if ( gyro_angler_velocity < -10  || 10 < gyro_angler_velocity )
		{
